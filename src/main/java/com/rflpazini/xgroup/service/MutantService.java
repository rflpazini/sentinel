...
public class MutantService {
  ...
  @Autowired MutantRepository mutantRepository;
  ...
  public Mutant checkIfDnaWasStoredOnDb(Mutant mutant) {
    Mutant newMutant =
        mutantRepository.findMutantByDna(mutant.getDna().stream().collect(Collectors.joining(",")));
      ...
      for (int x = 0; x < maxX; x++) {
        for (int y = 0; y < maxY; y++) {
          ...
          if (0 <= lastX && lastX < maxX && 0 <= lastY && lastY < maxY) {
            ...
            if (!genes.isEmpty()
                && genes.equals(dnaSequence[x + directionX][y + directionY])
